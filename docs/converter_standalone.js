...
class InvokeAIPromptResolver {
    ...
    convertAuto1111ToInvokeAI = function (inputPositive, inputNegative, resolverOptions = null) {
        ...
        targetConversionTable.forEach(regexPatternItem => {
            ...
            if (patternRecursive) {
                input = this.regexValueRecursiveReplace(input, regexPatternItem, ignoreNegativeParameters);
            } else {
                input = this.regexValueReplace(input, regexPatternItem, ignoreNegativeParameters);
            }
        });

        //Include tokens to the output
        var finalOutput = this.prepareOutput(input, inputPositive, inputNegative, false);
        return finalOutput;
    }
    ...
    convertInvokeAIToAuto1111 = function (inputPositive, inputNegative, resolverOptions = null) {
        ...
        //Include tokens to the output
        var finalOutput = this.prepareOutput(input, inputPositive, inputNegative, true);
        return finalOutput;
    }
        ...
        for (var i = loopCount; i >= 0; i--) {
            ...
            inputRegexArray.forEach(function (inputRegexItem) {
                ...
                replacements.forEach(function (replacementItem) {
                    ...
                    var powValue = typeof replacementItem.powValue !== 'function' ? replacementItem.powValue : replacementItem.powValue(resolverContext);
                    if (iterations > 0) {
                        if (powValue) {
                ...
                if (typeof patternOutput !== 'function' || patternOutput.toString().indexOf("regexGroups") === -1) {
                    if (typeof patternOutput !== 'function') {
                        ...
                        inputPositive = inputPositive.replace(regexExp, patternOutput(resolverContext));
                    }
                    if (!ignoreNegativeParameters) {
                        if (typeof patternNegativeOutput !== 'function') {
                            ...
                            if (patternNegativeOutput.toString().indexOf("regexGroups") === -1) {
                                inputNegative = inputNegative.replace(regexNegativeExp, patternNegativeOutput(resolverContext));
                            } else {
                                const regexNegativeGroups = inputNegative.matchAll(inputRegexItem);
                                inputNegative = patternNegativeOutput(resolverContext, inputNegative, regexNegativeGroups);
                            }
                        }
                    }
                } else {
                    ...
                    inputPositive = patternOutput(resolverContext, inputPositive, regexGroups);
                    ...
                    if (!ignoreNegativeParameters) {
                        if (typeof patternNegativeOutput === 'function') {
                            ...
                            inputNegative = patternOutput(resolverContext, inputNegative, regexNegativeGroups);
                        }
                    }
                }
    ...
    resolveTokensValues = function (inputValue, invokeAI = false) {
        ...
        regexCleaner.forEach(regexPatternItem => {
            ...
            var patternRecursive = regexPatternItem.recursiveCheck;
            if (patternRecursive) {
                output = resolverContext.regexValueRecursiveReplace(output, regexPatternItem, true);
            } else {
                output = resolverContext.regexValueReplace(output, regexPatternItem, true);
            }
